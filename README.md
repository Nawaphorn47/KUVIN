...
- **แผนที่จริง (Leaflet + OpenStreetMap + OSRM)** — `components/shared/MapView.jsx` แทน `MapPlaceholder` ทุกหน้าจอ:
  ...
  - พิกัดสถานที่ใน seed: หอสมุด/โรงพยาบาลสัตว์/หอพัก/ตลาด ตรวจกับ OpenStreetMap แล้ว ส่วนที่เหลือเป็น**ค่าประมาณ**
    รอบจุดกลางมหาวิทยาลัย (14.0230, 99.9739) เพราะ OSM ยังไม่มีข้อมูลอาคารเหล่านั้น — ต้องสำรวจ/แก้ให้ตรงก่อนใช้จริง
...
- ยังไม่มี: หน้าจัดการ user/driver แบบละเอียด (แก้ไขข้อมูล/ระงับบัญชี), หน้ารายงานสรุปตาม proposal
...
- พิกัดอาคารที่ยังเป็นค่าประมาณใน seed + หน้า admin สำหรับลากหมุดแก้พิกัดสถานที่
- หน้า "สถานที่โปรด", "ตั้งค่า", Chatbot ใน `apps/mobile` — ยังเป็น UI เปล่า/mock ไม่เคยต่อ backend
- QR พร้อมเพย์เป็นแค่ QR มาตรฐานให้สแกนจ่ายเอง ไม่มีการตรวจสอบอัตโนมัติว่าโอนจริงหรือยัง (คนขับกดยืนยันเอง)
