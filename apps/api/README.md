...
**ตัวเชื่อม SlipOK เขียนตาม "SlipOK API Guide" v1.13 (เอกสารทางการ) แต่ยังไม่เคยทดสอบกับ API key จริง** — ทดสอบด้วยสลิปจริงก่อนเปิดใช้ (โดยเฉพาะการอ่านข้อมูลผู้รับ ซึ่งรูปแบบการปิดบังเลขต่างกันตามธนาคาร)
ไม่ส่ง `log: true` (จะผูกกับบัญชีเดียวใน LINE LIFF ซึ่งไม่ตรงกับระบบที่มีคนขับหลายบัญชี) และไม่ส่ง `amount` (เทียบยอดเองเพื่อคุมข้อความที่บอกผู้โดยสาร) —
error โค้ด 1012/1014 ของ SlipOK (สลิปซ้ำ/ผู้รับผิด ฝั่ง SlipOK) จึงไม่มีทางเกิด เพราะเงื่อนไขที่ทำให้เกิดสองอย่างนั้นไม่ได้เปิดใช้
